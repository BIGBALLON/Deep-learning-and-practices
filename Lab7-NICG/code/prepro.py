    ...
    with tf.Session() as sess:
        ...
        for split in ['train', 'val', 'test']:
            ...
            for size in range(int(math.ceil(n_examples / 10000.0))):
                ...
                all_feats = np.ndarray([cur_size, 196, 512], dtype=np.float32)

                for start, end in zip(range(st, ed, batch_size),
                                      range(st + batch_size, ed + batch_size, batch_size)):
                    image_batch_file = image_path[start:end]
                    image_batch = np.array(map(lambda x: ndimage.imread(x, mode='RGB'), image_batch_file)).astype(
                        np.float32)
                    feats = sess.run(vggnet.features, feed_dict={vggnet.images: image_batch})
                    # all_feats[start:end, :] = feats
                    all_feats[start - st:end - st, :] = feats
                    print ("Processed %d(total %d) %s features.." % (end, n_examples, split))
